...
This repository contains MATLAB code files and shell scripts that you can use to submit jobs from a MATLAB or Simulink session running on Windows&reg;, Linux&reg;, or macOS operating systems to a IBM&reg; Spectrum&reg; LSF scheduler running on Linux.
...
For example, the `bsub` command to submit work and `bjobs` to query the state of submitted jobs.
...
Scheduler utilities are typically available if your MATLAB session is running on the LSF cluster to which you want to submit.
...
For example, if MATLAB should connect to `'lsf01.organization.com'` to submit jobs, set:
```matlab
c.AdditionalProperties.ClusterHost = 'lsf01.organization.com';
```

Use this option to connect to a remote cluster to submit jobs from a MATLAB session on a Windows computer to a Linux LSF cluster on the same network.
Your Windows machine creates an SSH session to the cluster head node to access the LSF utilities and uses a shared network folder to store job data files.
